...
**Last updated:** 2026-08-04 (G-300 instrument built) · **Phase:** 1 COMPLETE → 2 (persona discovery)
**Week:** 2 of 14
...
| Gold-300 annotation | ~~300 × 3 annotators~~ → **300 × 2** (deviation logged 2026-08-03) | **instrument built 2026-08-04; 0 items rated.** Sheets, guideline and scorer exist and are tested; what is missing is **people**. | RQ1 validation | 🔴 **now DECISIVE, not confirmatory** — and the binding constraint is no longer statistical. **Two annotators are available and one of them is the author**, who has read the log-odds lists and the cluster-representative reviews and cannot unsee them. **Recruiting one more independent annotator is worth more to RQ1 than any further statistic in this repository.** If Sabbir annotates anyway: no claim of *independent* human validation, only *partially independent*, disclosed in the abstract's limitations and the dataset card. Only **123 of the 300** are in region A (the split was stratified before G1 chose K); the split map stays frozen, all 300 are annotated, Gate 2 runs on the 123 and reports its power as a number. G1 found a reproducible 2-way split in a space with *no* cluster structure (silhouette 0.053, monotone gap, 100% HDBSCAN noise). Whether that split is an audience distinction or an artefact of cutting a continuum **cannot be settled by any statistic** — only by whether three annotators can reliably tell the halves apart. If they cannot, RQ1 is a negative result, which RQ1-C already recorded as publishable. Must be stratified on the **region-A** clustering, never the full-corpus one — that one is a corpus detector. G-300 is what decides whether cluster 1 (the 54/46 mixed cluster) is an audience persona or just the ambivalent reviews; nothing else can. |
...
1. 🔴 **Find one more independent annotator.** The G-300 instrument is built and tested (`data/annotation/`, `docs/g300_annotation_guideline.md`); what is missing is people. Two are available and one is the author — see the Gold-300 track. **This is now the binding constraint on RQ1, and it is not a statistical one.**
   Once a third person exists: hand out `docs/g300_annotation_guideline.md` and
   `data/annotation/g300_calibration_<who>.csv` (20 practice items, one
   discussion), then the 300 with **no further communication**, then
   `python -m src.annotate.g300_score`.
