...
**Last updated:** 2026-08-23 (**S5 Bangla computational post-run and registered
length-matched sensitivity analysis COMPLETE; three blinded human-evaluation
interfaces are frozen and annotation is in progress.**) The final, separately
scored archive contains the exact frozen **5,400/5,400** Bangla generation
surface: seeds 42/43/44 each contribute 1,800 condition-cases and all ten
conditions contribute 540. There are zero missing, extra, or duplicate case
keys; all 5,400 rows received outcome-only Verifier-B scores; the scored-cases
SHA-256 exactly matches the source-generation manifest; and no Verifier-B
score entered generation. The 20-cell main table, planned paired comparisons,
Goodhart trajectories, diversity/length diagnostics, and 20-cell
LaBSE-feature MAUVE sensitivity analysis are present in the audited post-run
bundle. MAUVE uses 270 generated and 270 real texts per condition × level, so
it is sensitivity evidence only, not a default-GPT2/MoP-comparable headline;
generated-text sentiment JS remains unmeasured because no independent,
registered scorer exists.**) **The exact thesis title remains open:** the
methodological constraint is settled (the controlled object is an
engagement-specificity **axis level**, never a persona/audience type/discrete
cluster), but Sabbir has not yet fixed whether the current words *"Audience
Simulation"* stay with the mandatory qualification *"axis-level-conditioned
response generation, not validated predictive audience modelling"* or are
replaced in the title itself. **S4.6 COMPLETE under two documented protocol
...
**−0.06272/−0.01805**, with B declining on **37/60** cases. The required
frontier/dynamics figure exists. The final failure taxonomy is the complete
**8/8 census**: off-topic **1**, post-hoc `other` **2**, and **5** with no
observable registered error. On Sabbir's explicit authorization, Codex coded
the eight emitted drafts and recorded its identity. Sabbir reviewed and
endorsed those labels, then explicitly waived independent Coder-B. This is
recorded as `single_coder_user_endorsed_protocol_deviation`, with
`independent_coder_b=false` and **agreement = null**; endorsement is not
misrepresented as independent agreement. The other deviation is the observed
8-case census instead of the specified 50. Prior S4.5b result: the
pre-registered quality-gain-per-call rule selects **tau*=0.4384071**:
Verifier-B quality **0.802219**, mean **2.000 LLM calls**, first-pass **0.6500**,
final acceptance **0.8667**, and **8/60 gave up**. alpha_lo = **0.640501** at one
call and maps to **row 3 RAG-only, not row 1 zero-shot** (the frozen prompt has
ten exemplars); explicit `FORCED_3` alpha_hi = **0.866272** at five calls; the selected
point buys **71.63%** of that range. All 60 cases have three attempts, the 240
cached calls are unique, attempt 1 is byte-for-byte the frozen length-controlled
archive, and the frontier recomputes exactly. Attempt 2 improves both A and B on
...
row. The main table is now 10 conditions. The first Kaggle gate caught that the
proposed external message sequence violated Gemma-3's native role alternation
before the 12B model loaded or any eval text existed. Rows 7a/7b now share a
valid `user → assistant → user` topology and move the identical critique bytes
between assistant and user content. **The remaining runner contracts are
also frozen before generation:** row 2 uses deterministic instance-randomized,
stratified R1 draws (10 examples/level/case; no query or outcome dependence);
row 5 uses symbolic tau **0.18166513482099075**,
which reproduces the neural gate's 39/60 first-pass rate without B. **Row 8 is
now a renamed full restart under a documented quota deviation:** hosted
`gemma-4-26b-a4b-it` through v1beta Interactions, seed 42, high thinking,
512-token output cap, structured PASS/FAIL + score + Bangla feedback, and no
temperature field. The account's observed limits are **30 RPM / 16K TPM /
14.4K RPD**; a 0.90 rolling request/token/day safety envelope is enforced.
Every prior Gemini-3.6 judge case and judge-conditioned retry is excluded from
the active checkpoint and retained in superseded archives; the other nine
conditions migrate unchanged with source provenance. This is explicitly a
hosted cross-generation **same-family** judge, not an independent-family
evaluator; sealed Verifier-B remains the sole final scorer. Client-side
validation rejects every missing/extra field;
and sampling
...
The CPU-only preflight now passes: exact 90 eval plots, 886-row R1-only RAG
manifest, a frozen 540-case instance-randomized 10+10 schedule, symbolic tau recomputation, all 10 conditions
and 3 seeds; it explicitly reports `verifier_b_loaded=false`. The earlier
Kaggle notebook was pinned to the Gemini 3.6 Interactions migration commit. The third
live gate had already passed
role rendering and schema validation before Google's 404 named 3.6 as the
required replacement. The first live one-base-case smoke then completed all 10
conditions and proved the Gemini/role/RAG/Verifier-B walls, but is **diagnostic
only and will not enter the S5 result**: rebuilding the RAG index rewrote its
tracked manifest timestamp, so every row honestly recorded the otherwise exact
runner as `febae25-dirty`. The runtime build now verifies against the committed
manifest without rewriting it, and restored JSONL files must have unique valid
keys from the exact clean runner commit before any model loads. The notebook is
repinned to the repair commit. Its next live gate passed runtime, Gemma roles
and Gemini, then correctly avoided rewriting the manifest but falsely refused
the index because the committed Windows `data\\rag\\...` path and Kaggle POSIX
`data/rag/...` path were compared as scientific content. All nine scientific
...
> **Current S5 state (2026-08-22):** Bangla computational post-run is complete
> in the audited `s5_bn_postrun_results_complete.zip` bundle. It has the exact
> **5,400/5,400** generation surface, **5,400** Verifier-B score rows, and a
> source-cases SHA-256 match between score manifest and cases. It includes 20
> condition × level rows for the main table, diversity/length diagnostics and
> LaBSE-feature MAUVE. The latter explicitly does **not** claim default-GPT2
> MAUVE comparability and is small-sample sensitivity analysis (`n=270` per
> distribution/cell). The generator is frozen: no S5 rerun is authorised.
...
| 17f | **S4.5b — neural gate + τ frontier** | ✅ **RUN 2026-08-20; independently validated and exactly recomputed.** The registered argmax selects **tau*=0.4384071**: Verifier-B quality **0.802219**, mean **2.000 calls**, first-pass **0.6500**, final acceptance **0.8667**, gave-up **0.1333 (8/60)**. alpha_lo **0.640501** (one call) → explicit `FORCED_3` alpha_hi **0.866272** (five calls); tau* captures **71.63%** of the achievable gain. 🔴 **Endpoint-name correction 2026-08-20:** alpha_lo is §5.1 **row 3 RAG-only**, not row 1 zero-shot; attempt 1 contains ten RAG exemplars. The number and tau* are unchanged, and the correction improves attribution because the frontier isolates loop-over-RAG gain. All **60×3** snapshots pass schema/range checks; 30/30 level balance, 240/240 unique cached calls, exact dev-plot set, and attempt 1 byte-for-byte matches the frozen length-controlled archive. Descriptive level-score permutation: Δ **+0.08289**, p **0.4687** — not detected, never “equal”. Runtime: T4, sklearn 1.9.0, transformers 5.15.0. ⚠️ Attempt 2 improves mean A/B; attempt 3 partially regresses. | `results/s4_tau_frontier.json`, `s4_tau_max_traces.jsonl`, `env_snapshot_s4tau_kaggle.json`; `data/generated/s4_tau_calls.jsonl` | 2026-08-20 |
| 17g | **S4.6 — loop dynamics + failure taxonomy** | ✅ **COMPLETE 2026-08-20 under two documented deviations.** Accepted stops **39/12/1**, gave-up **8**; emitted-best counts **41/16/3**. Attempt 1→2 mean Δ A/B **+0.08719/+0.08902**; attempt 2→3 **−0.06272/−0.01805**, with B down on **37/60**. A/B revision direction disagrees on **18/60** cases in each transition. Post-hoc B oracle **0.872402**; A-selected forced-three **0.866272** (97.36% of oracle gain), diagnostic only. The complete **8/8** failure census has off-topic **1**, `other` **2**, and **5** with no observable registered error; every other registered category is zero. Codex was Coder-A under Sabbir's explicit authorization. Sabbir reviewed and endorsed the labels, then waived independent Coder-B; therefore agreement is **null**, not fabricated. Deviations: 8 rather than 50 cases and single-coder/user-endorsed rather than independent double-coding. **Phase 4 complete.** | `results/s4_loop_dynamics.json`, `results/s4_failure_taxonomy.json`, `docs/figures/s4_loop_dynamics.svg`; local ignored final coding workbook | 2026-08-20 |
| 18 | **S5 Bangla — generation + computational post-run** | ✅ **COMPLETE 2026-08-22.** Exact **5,400/5,400** surface: 90 plots × 2 levels × 10 conditions × 3 paired replicate seeds. Every condition has 540 cases; every seed has 1,800; no duplicate or missing key. A separate, sealed Verifier-B step scored all **5,400** texts and its manifest source hash exactly matches the cases archive. Final generation evidence: commit `22124a8`, 7,068 local calls, 654 hosted calls, zero active transport failures, transformers 5.15.0, scikit-learn 1.9.0, T4. The post-run bundle contains the 20-cell master table, 9 planned paired comparisons, selection-aware Goodhart trajectories, diversity/length diagnostics, and a 20-cell LaBSE-feature MAUVE sensitivity analysis. **Boundaries retained:** B never entered generation; Gemma-4 is a same-family hosted judge and never final scorer; LaBSE-feature MAUVE is not default-GPT2/MoP comparable and `n=270` per distribution/cell is below MAUVE's recommended scale; generated-text sentiment JS is pending without an independent registered scorer. Next: blinded human evaluation/interface preparation, then final reporting. | `results/s5_main_bn_*`; `results/env_snapshot_s5_bn_kaggle.json`; `docs/s5_archive_manifest.md` | 2026-08-22 |
| 19 | **S5 Bangla — blinded human-evaluation preparation** | ▶ **PRE-REGISTERED, BUILT, AUDITED AND FROZEN 2026-08-23; annotation in progress, no responses ingested yet.** Consensus-first and alphaXiv full-paper review changed the draft design: removed an unregistered abstention option, enforced 5 cases per condition × level, seed allocation 2/2/1 per cell, minimal repeated plot/text exposure, three persistent blinded annotator codes and all 3 × 100 ratings. Sampling is outcome-blind and refuses any Verifier-A/B-conditioned selection. The builder verifies the frozen cases SHA-256 against the sealed score manifest; the scorer refuses missing/extra/duplicate/non-binary responses and pre-registers per-annotator/pooled accuracy, case-bootstrap CIs, raw three-way agreement and nominal Krippendorff alpha. Three adult native-Bangla university batchmates have provided informed consent; no monetary honorarium, refreshments provided; identities remain private and outside Git. The three HTML sheets were generated from clean commit `175b572`, passed balance/blinding audit and are frozen at commit `9870ff8`. A HEDS 3.0 preregistration snapshot records all known design/ethics fields and leaves only observed results, timing/feedback and institutional ethics determination pending. **Next: collect all three CSVs, then run the registered scorer.** | `data/annotation/s5_human_eval_bn/`; `configs/s5_human_eval_bn.yaml`; `src/annotate/s5_human_eval_{build,score}.py`; `docs/s5_human_eval_{consent_bn,heds3}.md` | 2026-08-23 |
| 20 | **S5 Bangla — deferred length-matched sensitivity analysis** | ✅ **COMPLETE 2026-08-23; no generation rerun.** The frozen same-plot × condition × replicate criterion (`|l0-l1| < 0.15*max`) retains **486/2,700 pairs**, but coverage varies sharply by condition: **9/270 (3.33%)** external-role and **11/270 (4.07%)** intrinsic self-critique versus **80/270 (29.63%)** blind resampling; other rows retain 30--70. Therefore matched-slice accuracies are sensitivity evidence only and may not rank conditions: the apparently high self-critique accuracies (0.944/0.955) rest on 9/11 selected pairs. Full 5,400-case results remain primary; coverage must accompany every matched number; no length-neutral control claim. Both 5,400-row input hashes are recorded and cases match the sealed manifest. | `results/s5_main_bn_length_matched.{csv,json}` | 2026-08-23 |
| 21 | **S5 Bangla — tracked Goodhart diagnostic figure** | ✅ **COMPLETE 2026-08-23; no generation rerun.** The two-panel figure keeps failure-selected attempt means visually separate from valid same-case adjacent transitions. The A-B gap widens for the neural loop by **+0.182802** (1->2, n=147) and **+0.114836** (2->3, n=67), and for neural+symbolic feedback by **+0.141481** (n=147) and **+0.145979** (n=58); the symbolic loop changes by **-0.042224** (n=193) and **+0.001396** (n=165). These are descriptive transitions among continuing failed cases, not estimates for all 540 cases. The manifest records the figure and both source-table hashes. | `results/s5_main_bn_goodhart_figure.png`; `results/s5_main_bn_goodhart_figure_manifest.json` | 2026-08-23 |
| 22 | **S5 Bangla — tracked corpus-level realism figure** | ✅ **COMPLETE 2026-08-23; no generation rerun.** The 20-cell figure reports exact word-count JS, under-four-word output rates and LaBSE-feature MAUVE without collapsing them into an unregistered composite. Length JS spans **0.153390--0.611987**; external-role critique level 0 has the largest short-output rate (**115/270, 42.59%**); LaBSE-feature MAUVE spans **0.010463--0.035995**. MAUVE remains n=270/cell small-sample sensitivity and is not default-GPT2/MoP comparable. Sentiment JS is explicitly unmeasured because no independent registered generated-text sentiment scorer exists. The figure manifest hashes all three audited input tables. | `results/s5_main_bn_realism_figure.png`; `results/s5_main_bn_realism_figure_manifest.json` | 2026-08-23 |
| 23 | **S5 Bangla — thesis-ready main/statistics tables** | ✅ **COMPLETE 2026-08-23; formatting only, no inference or generation rerun.** The rendered appendix contains all 20 Verifier-B condition×level cells and the exact nine planned paired comparisons against zero-shot, with effects, bootstrap CIs/p, BH q and McNemar p. All nine target-probability deltas are positive with CIs excluding zero; neural+symbolic feedback has the largest versus-zero-shot delta (**+0.256986 [0.215052, 0.298721]**). This does not test neural+symbolic versus neural-only: the frozen family contains only comparisons against zero-shot, and no post-hoc contrast is invented. The report carries the calibration-null, seed-blocking and pending-human-result constraints, and explicitly does not substitute for the separate registered dev mini-ablations. | `results/s5_main_bn_reporting_tables.md`; `results/s5_main_bn_reporting_tables_manifest.json` | 2026-08-23 |
...
> Sabbir** — the highest-risk item in this file. **Gold-300 round 1:** all
> **300 items received at least one rating; 598/600 individual ratings exist**
> (annotator A 300, B 298). Its reliability gate failed and RQ1-H replaced it
> (2026-08-08); no further rating round is scheduled. **Base-label
> reliability study:** 0 of 200, unscheduled.
...
| Gold-300 annotation | ~~300 × 3 annotators~~ → **300 × 2, both independent** (deviation logged 2026-08-03) | ✅ **ROUND 1 RUN 2026-08-05:** all **300 items** received at least one rating; **598/600 ratings** exist (A 300, B 298). Reliability gate failed (ordinal α **0.4970**), so Gate 2 was not run and no second rating round is scheduled. RQ1-H comparative judgement subsequently supplied the successful human validation. | RQ1 validation | ⚠️ **closed as an unreliable instrument, preserved as a result.** The two missing B ratings are reported rather than imputed. See pipeline rows 5k and 5m. |
...
| 🔴 What inviolable rule 7 costs | assumed negligible | **~18 macro-F1 points, measured.** CV **0.5150 → 0.6949** with IDF (**pilot only; unquotable as a main result**). F1's LOO delta **+0.1798** = 2.5× the CV SD; **variance falls as mean rises**. Worse, rule 7 as applied pushes the scorer onto the **gameable** presence families. **Current operational decision: rule 7 remains intact and `enable_f1` remains false. Nothing is pending unless a future proposal seeks to amend the rule; only then would a signed amendment be required.** |
...
| ~~**12a**~~ | ✅ **METHOD/TERMINOLOGY CLOSED 2026-08-10 — the engagement-specificity AXIS. Both *persona* and *cluster* are retired as scientific claims.** Sabbir delegated ("you can make the best decision"); the choice and reasoning are Claude's, recorded as such in `protocol.md`'s deviations log. **The literature moved the answer past both options that were on the table.** Pinto et al. (2026) obtain k=2, silhouette ≈0.31, ARI 0.999±0.001, sizes **50.6/49.4** on 8,360 psychometric respondents — numerically almost our region B (49.4/50.6) — and read it as *"geometric stratifications of a latent continuum rather than evidence for discrete subtypes"*, with the line **"Stability, therefore, is not equivalent to validity."** Cornelissen et al. (2026) publish a negative clusterability result and show a prior four-type typology was an artefact of k-means placing centroids on principal axes. So *persona* was already banned; ***cluster* does not survive either**, because the literature reserves it for structure we do not have (silhouette 0.053, monotone gap, HDBSCAN 100% noise). Permitted: **axis, gradient, the cut, level**; `cluster_k2` survives as a frozen *variable name* only. 🎁 **Our position is stronger than either paper's, and must be stated in these words: neither had human validation.** RQ1-H did — 0.78/0.84 vs 0.25 chance, length-matched, length heuristic below chance. The claim is therefore sharper than "the cut is arbitrary": **geometrically a line through a continuum, and people can nonetheless see it.** ⬛ Superseded histories retained below: ~~persona could be re-permitted after RQ1-H~~; ~~the levels were discrete clusters~~; ~~the pipeline used three personas~~. | Ch.1, Methods, all framing | ✅ **constraint closed** |
| **12b** | **Exact thesis title remains OPEN.** Current title: *A Neuro-Symbolic Multi-Agent Framework for Pre-release Audience Simulation in Bangla Cinema: A Verifier-in-the-Loop Approach*. The unresolved phrase is **“Audience Simulation”**: it can stay only with the explicit scope statement *“axis-level-conditioned response generation, not validated predictive audience modelling”*, or Sabbir can replace it in the title itself. **Non-negotiable regardless of wording:** the title/abstract may not imply discovered personas, audience types, discrete clusters, or film-level predictive audience modelling. “Multi-Agent Framework” remains the pre-defence-locked systems description. | Title, abstract, Ch.1 | 🔴 **Sabbir — exact wording** |
| ~~**12-old**~~ | ~~**Title and framing**~~ — historical formulation superseded by decisions 12a/12b above. Its terminology question is closed as **axis/level**; only the exact title wording remains open. | Ch.1, title, all framing | ⬛ **superseded** |
...
| ~~**9**~~ | ✅ **CLOSED 2026-08-20 — add row 9, a realized-compute-matched blind-resampling control.** Up to five nested independent samples use the exact RAG-only prompt; Verifier-A selects within the largest prefix whose measured generator FLOPs do not exceed the proposed loop on the same case. Verifier-B remains outcome-only; prefixes 1–5 and token/FLOP totals are all reported. Recent literature changed the design from a fixed-call best-of-N to measured compute matching. | §5.1 ablation, **the RQ2 headline** | ✅ closed; Codex decision under Sabbir's delegated authority |
| ~~**11**~~ | ✅ **CLOSED 2026-08-20 — add row 7b as a main paired control.** Rows 7a/7b share draft and critique bytes; SHA-256 is checked; only assistant/self versus `user` role changes and the revision seed is shared. No trained verifier is used. A row-6 gain over intrinsic self-critique is not attributed to trained external verification unless it also survives this role control and row 9. | §5.1, **the RQ2 claim** | ✅ closed; Codex decision under Sabbir's delegated authority |
...
| ~~**19**~~ | ✅ **CLOSED 2026-08-11; endpoint mapping corrected 2026-08-20 before Phase-5 generation.** The quality-gain-per-call argmax remains valid and selects tau*=0.4384071. 🔴 The frozen attempt-1 prompts contain ten RAG exemplars, so alpha_lo is **row 3 RAG-only, one call**, not row 1 zero-shot. The numeric alpha_lo=0.640501, alpha_hi=0.866272, frontier and tau* are unchanged; the clean estimand is loop gain over RAG. Both endpoint qualities are measured by Verifier-B, never A. | §4.5, RQ2, RQ5 | ✅ closed |
| ~~**10**~~ | ✅ **CLOSED 2026-08-11; wording corrected 2026-08-20.** Row 1 and loop attempt 1 use one renderer, so their base axis definition, plot, length clause and output instruction cannot drift. The complete prompts are intentionally different: row 1 has no exemplars; loop attempt 1/row 3 has ten. Therefore the guarantee is **shared-base parity**, not full prompt identity, and row 1 is not alpha_lo. | §5.1 internal validity | ✅ closed |
...
| ~~**20**~~ | ✅ **CLOSED 2026-08-20 by explicit waiver, not by satisfying the registered method.** Sabbir reviewed Coder-A, endorsed the labels, and said another annotator was unnecessary. Because this is neither blind nor independent, the finalized artifact records a single-coder/user-endorsed protocol deviation, `independent_coder_b=false`, and `agreement=null`. The limitation must remain beside the taxonomy in the thesis. | §4.6, Ch.4 failure analysis | ✅ closed with documented limitation |
...
| Rule table / symbolic scorer (§3.5) | ✅ **BUILT + FITTED 2026-08-11.** `src/symbolic/`, 22 tests. CV macro-F1 **0.5150 ± 0.0713** (resub 0.6570, majority 0.3926). **F1/IDF is disabled in the main pipeline under rule 7; no current blocker or pending action.** |
...
plots, the full **10-condition** ablation at **1,800 condition-cases/language** — **sequenced after
...
6. ✅ **`protocol.md` Phase 1–3 seal + supervisor signature complete.** Sealed
   2026-08-10; signature obtained 2026-08-11 (recorded as Sabbir's report; the
   signed paper is intentionally not stored in the repository).
...
12. ✅ **S4.5b tau quality--cost frontier run and validated.** Neural-only gate,
    symbolic diagnosis, Verifier-B post-loop evaluation, global tau-star under
    decision 19's pre-registered objective. Selected tau*=0.4384071 at 2.000
    calls and Verifier-B quality 0.802219; full frontier retained.
13. ✅ **S4.6 quantitative dynamics + frontier figure complete.** The report
    records non-monotone retry curves, per-level dynamics, A/B disagreement,
    editing magnitude and post-hoc oracle headroom.
14. ✅ **Failure taxonomy finalized under explicit deviation.** Complete 8/8
    census; single-coder/user-endorsed; no independent Coder-B and agreement is
    null. Coder-A's CSV/XLSX is local and ignored because it embeds plot text.
15. ✅ **Phase 5 pre-run design audit complete.** Decisions 9 and 11 are closed
    before generation; the literature moved cost matching from fixed calls to
    realized token/FLOP matching and promoted role relabelling to a main control.
16. ✅ **Bangla §5.1 runner, tests, generation and computational post-run are
    complete.** The frozen surface is exactly 5,400/5,400 cases: ten
    conditions × two levels × 90 plots × three seeds. Verifier-B remained
    outcome-only. **Next: Phase-5 human-evaluation/interface preparation and
    the still-registered post-run analyses; no Bangla generator rerun.**

⚠️ **Still Sabbir's:** decision 12b (exact title wording), decision 13
(Unicode-reporting treatment), decision 5 (stylometry vs MGT framing), the
CC BY-SA attribution checklist, and — the highest-risk item in this file —
**reading the six base papers, none of which Sabbir has read.** The title's
methodological bound is already fixed: axis/level language only, never
persona/audience-type/discrete-cluster claims. Rule 7 is also not an open
action: `enable_f1` remains `false`; an amendment is needed only if a future
proposal tries to turn it on.
...
| `s5_main_bn_cases.jsonl` / `s5_main_bn_verifier_b_scores.jsonl` / `s5_main_bn_scored_cases.csv` / `s5_main_bn_score_manifest.json` | frozen 5,400-case Bangla surface, separate outcome-only B scores, joined score table, and source-hash contract | ✅ current — Verifier-B absent from generation |
| `s5_main_bn_master_table.csv` / `s5_main_bn_paired_statistics.csv` / `s5_main_bn_analysis.json` | 20 condition × level cells and 9 planned paired bootstrap/BH comparisons | ✅ current |
| `s5_main_bn_goodhart_*` | attempt-level descriptive rows, selection-aware paired transitions and report | ✅ current — later attempts are failure-selected; paired transitions are the valid descriptive view |
| `s5_main_bn_diversity.csv` / `s5_main_bn_length_js.csv` / `s5_main_bn_realism_preflight.json` | Self-BLEU, short-text rates and exact length-distribution diagnostics | ✅ current — no generated-sentiment claim |
| `s5_main_bn_labse_mauve.csv` / `s5_main_bn_mauve_report.json` | 20-cell LaBSE-feature MAUVE sensitivity analysis | ⚠️ current only as small-sample sensitivity; not default-GPT2/MoP comparable |
| `env_snapshot_s5_bn_kaggle.json` | exact final generation runtime extracted from the hash-registered 5,400-case checkpoint | ✅ current — commit `22124a8`, transformers 5.15.0, sklearn 1.9.0, Tesla T4 |
