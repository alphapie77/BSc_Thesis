...
| 18 | **S5 main — contracts, preflight + generation orchestrator** | 🔨 **HOSTED GEMMA-4 REPLACEMENT BUILT; LIVE PREFLIGHT PENDING.** Ten conditions and seeds 42/43/44 remain frozen, but quota exhaustion forced row 8 to restart under the explicitly renamed `gemma4_26b_a4b_judge_loop`. It uses hosted `gemma-4-26b-a4b-it`, seed 42, high thinking, 512 max output tokens and the unchanged exact structured verdict. Account limits observed in AI Studio are 30 RPM / 16K TPM / 14.4K RPD; the runner enforces a 0.90 rolling request/token/day envelope. The migration accepts only the clean `510a95c` checkpoint, preserves the other nine conditions and shared initials with source provenance, and moves all Gemini-3.6 calls/cases and judge-conditioned retries into named superseded archives. Same-family preference is a registered limitation; Verifier-B remains sealed and final-score-only. Earlier dirty smoke remains excluded. CPU preflight still resolves 90 eval plots, 886 R1-only rows, the 540-case static schedule and symbolic tau 0.18166513482099075 = 39/60, with B absent. **328 full-suite tests pass.** The latest user run log reached 100/200 condition-cases before HTTP 429, but its exported archive has not yet been ingested locally, so that progress count is not promoted to a verified artifact fact. Next: pin/push notebook, attach that clean checkpoint, run migration + Gemma-4 live preflight, then one smoke case. | code/config/tests + `notebooks/s5_main_bn_kaggle.ipynb` | — |
