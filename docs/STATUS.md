...
|---|---|---|---|---|
| Bangla plot synopses | 130 (30 dev + 100 eval) | **132 harvested** (of 2,995 candidates) — scraped, not hand-written. Over target, but **2 are biographies of people** (অপর্ণা সেন, অনুস্মৃতি সরকার) that passed every mechanical gate, leaving exactly 130 with zero slack. Person-article veto added; growing the harvest before sampling | S6 evaluation | 🟡 — Scraping removes both the 26-day cost *and* two biases: the experimenter's register in the inputs, and hand-selection of famous films. **New obligation:** bn.wikipedia is CC BY-SA 4.0 — attribution + revision ids must be in the dataset card before publication |
