...
| Base-paper reading | 5 Tier-1 | **1** — `huang2024selfcorrect` entry filled 2026-08-01 (⚠️ read by Claude, not by Sabbir; see the provenance note on the entry) | Ch.1, Ch.2 | 🔴 **highest now** — plots are frozen and S2 is done, so nothing else is competing for the time |
