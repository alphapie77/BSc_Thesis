...
> **Draft standing.** This chapter reports the completed Bangla arm only. The
> English mirror is deferred and is not represented by a partial comparison.
> Exact values come from the audited Phase-5 result bundle; rounded tables below
> are presentation views, not recomputed analyses.

## 6.1 Experimental questions and scope
...
The primary outcome is Verifier-B target probability. Binary target-match
...
Verifier-B target-probability effects, and every 95% paired-bootstrap interval
excludes zero after reporting the registered family. The largest effect belongs
...
bootstrap p-value is 0.00019998 and the Benjamini–Hochberg q-value is 0.00019998;
...
evaluator remains valid [@shihab2025est]. The independent scorer wall therefore
...
balanced 100-item subset under blinded condition labels. All 300 registered
judgments passed the ingestion gate. Pooled target-match accuracy is 0.9133
(case-bootstrap 95% CI [0.8667, 0.9567]). Annotator accuracies are 0.91, 0.93,
and 0.90. Raw three-way agreement is 0.88, and nominal Krippendorff alpha is
0.8405 (item-bootstrap 95% CI [0.7473, 0.9200]).
...
default GPT-2/MoP MAUVE [@pillutla2021mauve]. Sentiment JS remains unmeasured
...
- **RQ4:** Unanswered in the present thesis draft. The matched English mirror is
  deferred; no cross-lingual difference is estimated.
- **RQ5:** Supported as a diagnostic finding. Same-case A–B gaps widen across
