...
| 2026-08-11 | ✅ **STANDING RULE from Sabbir: a hand-written constant is not acceptable — every decision number must carry a reason. Enforced by `src/common/check_constants.py`, not by memory** | Sabbir: *"hate likha thakle hbe na. karon thakte hobe."* Prompted by §4.2's Critic weight `0.6xVerifierA + 0.4xsymbolic`, which appears in the pipeline spec with **no derivation anywhere**. A repo-wide audit followed. New checker classifies every numeric config constant into three tiers and enforces only the first: **DECISION** (changes a verdict — must have a reason), **KNOB** (engineering default — warned, never enforced), **ASSERTION** (`expected_n`, `train_n`, the S0 claimed counts — these are the *opposite* of hand-set, they exist to fail loudly if the data moves, and are skipped). A reason counts if it is an inline comment, a comment block above, or **the key appearing in `protocol.md`** — the third being the one that matters, since the reason belongs here and the config may simply point at it. Unclassified keys default to DECISION: when we do not know what a number does, the safe default is to demand a reason, not to wave it through. | 🔴 **Result of the first run: 97 constants carry a reason, 29 DECISION-tier do not.** The problem was **not confined to 0.6/0.4**. Three of the worst are `independent_at_or_above` (0.60 and 0.20), `explained_at_or_above` (25.0) and `strong_at_or_above` (0.45, 0.70) — thresholds that decide whether a test passes, which appear **zero times in `protocol.md`** and exist only as bare numbers in YAML. Also unjustified: `persona_claim_fails_above: 0.60` (in **four** configs), `not_sentiment_aligned_below: 0.20` (three), `surface_auc_headline: 0.80`, `dominated_at_or_above: 0.75`, `confounded_at_or_above: 0.65`, `r1_fraction: 0.5`, `min_words: 3`. **Why this is worse than it looks:** a bare number in a YAML file reads as authoritative as a derived one, so nothing about the file's appearance distinguishes a threshold with a literature behind it from one somebody typed. That is exactly how `0.6/0.4` survived into a normative spec. ⚠️ **The 29 are NOT fixed in this commit and must not be fixed by Claude alone** — several were Sabbir's calls and CLAUDE.md forbids guessing at his reasoning and writing it down as his. Each needs either a recorded reason or a derivation; where the reason is Sabbir's, he supplies it. The checker exits 1 until then, which is the point. |
