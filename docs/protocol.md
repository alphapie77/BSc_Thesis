...
| 2026-08-11 | **English arm formally DEFERRED, not dropped — invoking the charter's own cut rule** | Pipeline §1.2 (English Arm Charter), §3.1 and §3.2 specify a full mirrored English pipeline; the Phase 3 deliverable is *"4 trained verifiers (A/B × bn/en)."* **Nothing English exists** — no `imdb`, no `mpst`, no `distilroberta`, in `src/`, `configs/`, or this document. Phase 3's pre-registration is Bangla-only and never said so. | **The charter anticipated this and pre-authorised it:** *"English arm exceeding one week → **cut to fertility + zero-shot reference only**."* That clause is invoked deliberately, in advance, rather than allowed to happen by drift. **What this costs, stated plainly: RQ4 (Δ_bn > Δ_en) cannot be answered in its strong form**, and the cross-lingual claim reduces to the fertility covariate plus a zero-shot English reference — which the charter already registered as an acceptable landing point. **Mirror-never-merge (inviolable rule 8) is unaffected** and remains binding if the arm is restored. Recorded now because a deferral that is never written down becomes, at viva, a deliverable that was silently missed. |
