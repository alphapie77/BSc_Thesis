...
3. **LLM judge (row 8).** Use stable model ID `gemini-3.6-flash` through the
   ...
   **Superseding amendment, 2026-08-21 (after the first clean partial run
   stopped on provider quota):** the retained row-8 condition is renamed
   **`gemma4_26b_a4b_judge_loop`** and restarts in full with Google's hosted
   `gemma-4-26b-a4b-it` through the same v1beta Interactions surface. The old
   Gemini-3.6 judge cases, judge-conditioned Writer retries and raw responses
   are excluded from the active checkpoint and preserved in explicitly named
   `superseded` JSONLs; the other nine conditions and shared RAG initials are
   migrated unchanged with both source and migration provenance. No 3.6 and
   Gemma-4 verdicts may coexist under one condition. Gemma-4 freezes seed 42,
   `thinking_level=high` (the model exposes only minimal/high), a 512-token
   output cap, the same exact-key JSON schema, at most three attempts and the
   same all-fail rule. AI Studio showed **30 RPM / 16K TPM / 14.4K RPD** for the
   account; the runner enforces a 0.90 safety fraction and a rolling realized-
   token gate. This is a **hosted cross-generation, same-family judge**, not an
   independent-family evaluator: Pombal et al. (`pombal2026selfpreference`)
   show that rubric judges can over-credit outputs from their own model family,
   including family-level effects for Gemma. The final outcome remains scored
   only by sealed Verifier-B, outside every loop. Yang et al.
   (`yang2026judgechanges`) show why the replacement is a new measuring
   condition rather than a transparent model upgrade.
...
|---|---|---|---|
| 2026-08-21 | S5 row 8 — quota-forced judge replacement after partial generation | The active condition becomes `gemma4_26b_a4b_judge_loop`, using hosted `gemma-4-26b-a4b-it`, seed 42, high thinking, 512 max output tokens and the unchanged structured PASS/FAIL + score + Bangla-feedback contract. Every Gemini-3.6 judge result and its judge-conditioned Writer retries are removed from the active checkpoint and preserved in explicit superseded archives. The other nine completed conditions and shared initial drafts are retained byte-for-byte with source plus migration provenance. The full Gemma-4 row restarts from zero; no mixed-judge row is permitted. The account's observed limits are 30 RPM / 16K TPM / 14.4K RPD; the runner uses a 0.90 rolling request/token/day safety envelope. | Gemini 3.6 returned HTTP 429 after the clean run had progressed, making the registered model infeasible on this account's free quota. Google officially exposes Gemma-4-26B-A4B through the Gemini and Interactions APIs at no token charge on the free tier. The replacement was selected by Sabbir before any Gemma-4 judgment existed. Literature search changed the implementation: `yang2026judgechanges` treats judge replacement as measurement replacement, forcing a renamed full-row restart; `pombal2026selfpreference` finds family-level preference in rubric judging, forcing the same-family limitation and preserving sealed Verifier-B as the only final scorer. |
| 2026-08-21 | S5 first live smoke — tracked-manifest/provenance and resume gate | BN002/L0 completed all 10 conditions and validated the live Gemini 3.6 request, role controls, R1-only index and absence of Verifier-B. Inspection of the exported zip then found every row stamped `febae25-dirty`: runtime index construction had rewritten the tracked S4 manifest solely to refresh provenance. These 10 rows are retained only as a diagnostic and excluded from the S5 result. Runtime index construction now compares its scientific invariants to the committed manifest without rewriting it; the host-local `persist_dir` spelling is deliberately excluded because the committed Windows `data\\rag\\...` and Kaggle POSIX `data/rag/...` locate the same config-relative artifact. Before loading LaBSE or the Writer, restored local-call, Gemini-call and case JSONLs must parse fully, contain unique keys, and carry the exact clean runner commit; malformed, dirty or cross-commit checkpoints are refused. | A `-dirty` suffix must mean uncommitted tracked state, not be waived because the diff looks harmless after the fact. Mixing the diagnostic checkpoint into a repinned full run would also create a multi-commit result silently. This repair changes no prompt, seed, threshold, model, sample, condition or observed selection rule; it makes the existing provenance/resume contract executable. The next live gate exposed an over-strict whole-dictionary comparison: only `persist_dir` differed by OS path separator while all nine scientific invariants matched. No literature search was used because these decisions are entailed directly by cross-platform path semantics, the repository's recorded provenance rules and the inspected artifacts, not a scientific method choice. |
...
| 2026-08-15 | 🔴 **CUDA OOM on the dev-plot run: retrieval and generation were competing for the same 16 GB, and an unquantised load could not announce itself** | `run_devplots.py` built the `Researcher` first, and `sentence-transformers` places LaBSE on the GPU by default, so the encoder held VRAM for the whole run while a 12B generator loaded beside it. Two changes, both structural rather than a retry: (i) **all retrieval runs first, on CPU, and the encoder is released before the generator loads** — attempt 1 has no feedback, so retrieval depends on nothing generation produces and interleaving them only meant holding two models; (ii) `LocalWriter` now **verifies after loading that the requested quantisation actually happened** and raises if not. | 🔑 **The second change is the one that matters beyond this crash.** A `quantization_config` that is ignored does not fail — the model loads in fp16 at ~3.4× the memory and on a *different numerical path from every other generation in the archive*, and the only symptom is an OOM somewhere further on, pointing at CUDA rather than at the cause. Had the card been large enough, it would not have crashed at all: it would have quietly produced generations that are not comparable with the rest. A notebook preflight (bitsandbytes present, both GPUs empty) is added so the check happens before a 40-minute run rather than during it. |
| 2026-08-15 | 🔴 **The OOM recurred after the retrieval fix: `transformers` DROPS `quantization_config` silently when bitsandbytes is unavailable, and loads fp16 instead** | Second OOM, same run, different cause — retrieval was already off the GPU and the encoder released, yet a 12.19B model still filled a 14.5 GiB card, i.e. it was loading unquantised. `transformers` does not raise for a missing bitsandbytes; it discards the 4-bit config and proceeds. The verification added earlier that day ran **after** `from_pretrained`, so the OOM always won the race. Moved: availability is now checked **before the load**, with a message naming the package rather than CUDA. | 🔑 **The recorded lesson is about where a check goes, not that a check exists.** A post-hoc verification of something that can exhaust memory during the operation it verifies can never fire. The same guard, moved twenty lines earlier, turns a 90-second wait and an eleven-frame CUDA traceback into one legible sentence. ⚠️ **And the silent-fp16 path remains the real hazard**: on a card large enough it would not crash at all — it would produce generations on a different numerical path and nothing would say so. |
| 2026-08-15 | 🔴 **ROOT CAUSE of both dev-plot OOMs: `transformers 5.0.0` (Kaggle's stock build) does not apply a 4-bit `quantization_config`, and the runner notebook had dropped the pilot's `pip install -U transformers`** | bitsandbytes was present and `is_bitsandbytes_available()` returned **True in the failing session**, so availability was never the test. The difference is the library version: the pilot ran on **transformers 5.15.0** (`results/env_snapshot_s4_kaggle.json`) and 4-bit worked; the dev-plot notebook, rebuilt from scratch, omitted the `-U transformers` line and inherited Kaggle's stock **5.0.0**, which loads fp16 and says nothing. Fixed in three places: the install cell upgrades and says why, the preflight compares against the pilot's recorded version, and `LocalWriter` refuses to load below it. | 🔴 **Third time this project has been bitten by Kaggle's stock library version** — S3.2 crashed at arm 6 of 7 on transformers 5.x, and its remedy was to pin `transformers<5`. **The lesson that did not transfer: the pin lived in one notebook, not in the code.** A rebuilt notebook silently lost it, and the guard that would have caught it is only being written now. ⚠️ **Environment parity is not hygiene here**: Coakley et al. (2022) measure >6 pp of accuracy variation from environment alone, and the generations this run produces are the substrate for `w` and τ. |
| 2026-08-15 | ⚠️ **All 120 dev-plot generations succeeded; the run then exited non-zero on the report line — `write_text_lf(text, path)` with the arguments reversed** | The signature is `write_text_lf(path, text)` and the call also passed a `config_path=` the function does not accept. **No generation was lost**: every one is appended to the JSONL as it completes, which is the whole point of that design, so the fix is to re-run the reporting, not the generation. A test now binds the real signature with `inspect` rather than reading the call site. | Recorded because of *where* it failed: the last line of a 40-minute run, after the expensive part had fully succeeded. The append-as-you-go archive turned what could have been a lost run into a re-run of the cheap step — the same property that absorbed the 21/80 OOM three days earlier. |
| 2026-08-16 | 🔴 **S4.dev RAN (120/120) and the pre-registered length diagnostic PASSED while the confound it exists to catch is present in its strongest possible form — the rule was written one-directional and the observed effect is inverted** | Measured on the 120 attempt-1 generations: level-1 outputs are **far LONGER** than level-0 — bn **13.5 → 38.3** mean words, en **6.3 → 40.3** — so the registered test (*"if level-1 is SHORTER than level-0 by ≥ the corpus gap of 4.27 words"*) returns `GAP_BELOW_CORPUS`, i.e. a pass. **Length alone separates the two levels essentially perfectly: AUC 0.9894 (bn) and 1.0000 (en)**, computed here and labelled exploratory. Truncation (2 bn, 3 en, all at L1) biases the L1 mean *downward*, so the true separation is if anything larger. **The diagnostic's verdict is therefore reported as UNINFORMATIVE, not as a pass**, and no axis-control claim may rest on it. | 🔑 **The pre-registration failed in a specific, recordable way: it fixed a direction as well as a magnitude.** It was written from the corpus signature (human level-1 is 33% *shorter* yet richer) and assumed the generator would reproduce that relation. `kapur2026length` — already cited in this protocol, and the stated reason `w` is fitted on generations rather than dev-82 — says exactly that the length/specificity relation is *"flat or reversed in machine text"*. **The paper that predicted this outcome was in our own bibliography while the test was written to miss it.** ⚠️ No remedy is adopted here: the amendment is a design decision and goes to Sabbir with a search behind it. What is fixed now is only the reporting — magnitude on \|gap\| plus the length-only AUC, both printed beside every axis-level number. |
| 2026-08-16 | ✅ **REMEDY REGISTERED: a length-controlled condition is added as a FACTOR, and the direction-free confound measure replaces the failed diagnostic** | Three changes, registered before the condition is run. **(1)** `configs/s4_devplots_lenctl.yaml` repeats S4.dev with exactly one thing changed — one sentence appended to the prompt, **identical at both levels**, capping the comment at **20 words**. 20 is derived, not chosen: region A's level 0 averages **13.12** words and level 1 **8.85**, corpus median **8**, so the cap sits above both means and never asks for text shorter than the humans wrote. A per-level cap is refused explicitly — it would re-tie length to level by hand. **(2)** The free-length archive is **retained as a condition, not superseded**: it is the measurement of what the generator does unconstrained, which is to read *specific* as *long*. **(3)** Every axis-level report now carries the **content-blind length-only AUC** and the **matched-pair count**, and the `length_confound` verdict (`LENGTH_RECOVERS_LEVEL` ≥ 0.90) replaces `length_diagnostic` for any axis-control claim. **Pre-committed: if the AUC stays ≥ 0.90 and the matched slice stays empty, the control FAILED and is reported as failed.** | **Searched before deciding** (alphaXiv; Consensus quota exhausted to 1 Sep). ⚠️ **Provenance: Sabbir delegated (*"research kore dekho ki korle valo hbe"*, then *"amar shiddhanto kn abar"*); the choice, the 20-word cap and the reasoning are Claude's, endorsed not authored.** 🔑 **The search supplied the decision, not a justification for one already made.** `2607.18508` audits a preference benchmark with content-blind probes and recommends *both* generation-time length control *and* evaluation-time matched slices, stating that neither alone establishes a score is grounded. Applying their matched criterion to our 120 free-length generations returned **0 matched pairs in both arms** — in the en arm the ranges do not overlap at all — which is what closed the report-only option: the slice a length-neutral claim needs **does not exist**, so this was never a choice between controlling at generation time or at reporting time. ⚠️ Counterweight kept in view: `2601.01768` finds LLMs track their own output length poorly, so the clause is expected to shift the distribution rather than enforce a bound — hence the achieved lengths and slice size are measured and printed, and a failed control has a pre-committed name. 🔑 **Note what the clause is not:** `docs/axis_definition.md` §3 already tells the model that short and long comments are both normal at both levels. It read that and ignored it. This escalates a permission to an instruction; it introduces no new claim about the construct. |
| 2026-08-17 | ⚠️ **The length-controlled condition RAN: it made a matched slice exist but did NOT remove the confound. Verdict `LENGTH_RECOVERS_LEVEL` in both arms** | 120/120 generated, 0 truncated (was 5), 0 verbatim copies. Mean words: bn **11.5 → 16.2**, en **10.0 → 19.2** — against free-length's 13.5 → 38.3 and 6.3 → 40.3, i.e. the level gap fell from **24.8 to 4.8** words (bn) and **34.0 to 9.2** (en), and both levels are back at corpus scale (13.12 / 8.85). **But the length-only AUC barely moved: 0.9894 → 0.9111 (bn), 1.0000 → 0.9928 (en).** Matched pairs went **0 → 8** (bn) and **0 → 2** (en). Level 1 is still the longer of the pair in **25/30** (bn) and **29/30** (en). | 🔑 **Why the two measures disagree, stated because it is the methodological point:** AUC is rank-based and the clause compressed the *magnitude* of the gap without disturbing its *order*. A five-fold reduction in mean difference buys almost nothing on a rank statistic while buying the entire matched slice. **Both numbers are correct and they answer different questions** — "is the level readable from length?" (yes) versus "can a length-neutral comparison be constructed?" (now marginally, bn only). |
| 2026-08-17 | 🔴 **SECOND pre-registration defect, self-reported: the failure criterion was written as a CONJUNCTION and only one half fired, so the registered rule returns no verdict** | The 2026-08-16 registration reads *"if the AUC stays ≥ 0.90 **and** the matched slice stays empty, the control FAILED"*. Observed: AUC ≥ 0.90 in both arms (true), matched slice non-empty (false). **The rule as written does not fire, and it is recorded that it does not fire rather than being read in whichever direction suits.** The outcome is stated in plain terms instead: **the control partially succeeded — it made a length-matched analysis possible and did not make the level unreadable from length.** | **Same shape as the defect logged yesterday, one day later and by the same author.** Yesterday's rule fixed a *direction* the data then inverted; today's fixed a *conjunction* the data then split. Both were written in the hour before the run, from an expectation of how the result would look. ⚠️ **The lesson recorded for the remaining pre-registrations (τ, §4.6, Phase 5): state outcome bands over a single quantity, not compound conditions over several** — a compound rule has failure modes that are invisible until the data lands in the gap between its clauses. |
| 2026-08-17 | ✅ **The length clause becomes STANDARD for all subsequent generation; free-length is retained as a registered secondary condition; the length-matched analysis MOVES to Phase 5** | Three consequences, registered together. **(1)** Every Phase 4/5 generation uses the length-controlled prompt — it is uniform across all §5.1 rows, so ablation comparisons stay attributable, and prompt parity (§S4 decision 5) still holds by construction since the clause lives in the one shared template. **(2)** The free-length archive stands as the condition measuring unconstrained behaviour, and its §5.4 realism finding (the generator inverts the corpus's length/specificity relation) is unaffected. **(3)** A length-matched comparison on **8 pairs** (bn) and **2** (en) is not worth reporting as an analysis; at Phase 5's **90** eval-plots the same matched rate yields ~24 pairs (bn), which is. The matched analysis is therefore **deferred to Phase 5, not abandoned**, and the dev-plot slice sizes are reported as the reason. | ⚠️ **Provenance: Sabbir delegated; the choice and reasoning are Claude's.** 🔑 **What is explicitly NOT done: the prompt is not tuned again.** One length clause was registered, run, and measured; iterating on it until the AUC falls would be selecting a prompt on the outcome it is meant to be tested against, which is the failure `CLAUDE.md` forbids in its own words (*"never tune, filter, or reframe to make a number look better"*). 🔴 **The standing limitation, to appear in Ch.5 and beside every axis-level number:** in generated text the axis level remains recoverable from length alone (AUC 0.91–0.99), so **no claim of length-neutral axis control is made anywhere in this thesis.** The construct claim rests where it always did — RQ1-H's human validation on *corpus* text (0.78/0.84 against 0.25 chance, length-matched to within 2 words, with a length heuristic scoring 0.16, below chance). |
| 2026-08-17 | 🎁 **Non-Bangla script leakage measured across both conditions: it did not get worse under the length clause, and its composition changed** | Free-length: **15/120** generations carry non-Bangla letters (bn-L1 6, en-L0 1, en-L1 8). Length-controlled: **12/120** (bn-L0 2, bn-L1 4, en-L0 3, en-L1 3). Scripts seen across the two runs: Latin, Devanagari, Tamil, Telugu, Cyrillic. The concentration at level 1 and under English prompts weakens in the controlled run, and the totals are within a few generations of each other. | Recorded because the opposite was suspected: shorter outputs looked, on a hand-read of the progress log, like they might leak more. **They do not — 12 versus 15 — and writing down the suspicion before checking is what makes the check worth anything.** ⚠️ n is 120 per condition and the difference is 3 generations; this is a description, not a test. The §3e prompt-language factor now has three data points (pilot 1/20 vs 4/20; free 6+1 vs 8; controlled 6 vs 6) and they no longer point the same way — reported as unsettled rather than as a trend. |
| 2026-08-17 | ✅ **S4.5a built: `w` is fitted as a curve, and the BASELINE for it is the length-only probe rather than chance** | `src/eval/fit_w.py` + `configs/s4_w.yaml` + 9 tests (**269 total**). 21-point grid over the **full** [0,1] range — not §5.1b's old 0.5–0.8 grid, which presupposed the answer lay in the neural-heavy half and excluded both endpoints the marginal-value test is defined against. Held-out test is **grouped by plot** (5 folds, 6 held-out plots each): both levels of a plot share a synopsis and ten exemplars, so an ungrouped split would leak the plot into its own test fold. Scores are computed **once** per generation and reused across the grid — the mixture is a weighted sum of two fixed numbers. Both archives are scored, so the curve's shape can be compared across a factor known to move the confound. | 🔑 **Two things recorded because they are the weak joints, not because they are solved.** **(i) The label is the level that was REQUESTED**, so every number measures agreement with the instruction, not whether the text sits at that level — the generator's compliance is assumed and cannot be checked without annotation. Right quantity for fitting `w` (rewarding compliance *is* the Critic's job); wrong quantity for claiming axis control, and the report says so in a banner. **(ii) The baseline is not 0.5.** S4.dev-LC measured a word count alone recovering the requested level at AUC **0.9111 / 0.9928**, so a hybrid score that cannot beat counting words has not earned its place. That comparison is printed beside every curve rather than left for a reader to reconstruct. ⚠️ A subtlety pinned by test: the Critic returns **P(y = target_level)**, so comparing raw scores across classes would make a *perfect* scorer look like chance — `p1` is reconstructed before any AUC. |
| 2026-08-16 | 🎁 **A second, unregistered finding from the same run: generated text does not reproduce the corpus's length signature at all, which is a §5.4 realism result rather than a defect** | The human corpus's level-1 is **shorter and lexically richer** than level-0 (8.85 vs 13.12 mean words, ~18% richer per 1k tokens — `results/s2e_regionA_k2_profile.md`). The generator inverts this: it renders "specific" as "long". Corpus median is **8 words**; generated level-1 is **37–40**. **Verbatim exemplar copying: 0 of 120** (the Groq pilot had produced an exact corpus copy, so this was worth counting). Non-Bangla script leakage is concentrated at level 1 and under English prompts: bn-L0 **0/30**, bn-L1 **3/30**, en-L0 **1/30**, en-L1 **6/30** — one generation carried **Tamil** script. | Recorded now, before any of it becomes convenient. The realism gap is the instrument §5.4 already exists for, and the leak asymmetry is the second data point for the §3e prompt-language factor after the pilot's 1/20 vs 4/20 — **same direction, larger n**. ⚠️ Both are single-run observations on 30 plots and are labelled as such. |
| 2026-08-20 | ⚠️ **S4.5b completed all 60 forced-three loop cases, then failed at Verifier-B load because the Kaggle transport root supplied the wrong `config.json`** | The failure occurred after every `trace 01/60` … `trace 60/60` line and before `s4_tau_max_traces.jsonl` was written. The append-only `s4_tau_calls.jsonl` in the live Kaggle session retains the expensive Writer/Reflector calls, so the run resumes rather than regenerates. The local trained directory's real config has `model_type: electra`; the mounted dataset root's file did not. Verifier-B's exact seed-42 `save_pretrained` config is now committed as `configs/s3d_verifier_b_hf_config.json`, and the evaluator constructs the architecture from that registered config while `from_pretrained` still checks every uploaded weight tensor against it. | 🔑 **Model architecture is artifact provenance, not dataset-transport metadata.** A folder can contain the correct 442 MB weights beside the wrong generic `config.json`; discovering that only after 60 cases makes the check far too late. Verified before retry: the registered config loads the real local weights as a 110,618,882-parameter `ElectraForSequenceClassification`, tokenizer size 32,000, and produces bounded end-to-end scores. No fitted parameter, generation, threshold rule, or scientific method changed. |
| 2026-08-20 | ⚠️ **S4.6 can audit 8 three-time gate failures, not the specified 50; all 8 are retained and no cases are manufactured** | §4.6 requests a hand-coded taxonomy of 50 three-time failures. The frozen 60-case run has **8** cases that fail Verifier-A at all three attempts. The coding universe is therefore the complete observed set (8/8), with the registered six-category-plus-`other` sheet and independent double-coding requirement unchanged. S4.6 quantitative dynamics may run now, but the taxonomy and the step itself remain open until the second coding exists. | This is a data-imposed shortfall, not a sampling choice. Duplicating failures, lowering tau after selection, or relabelling Verifier-B disagreements as extra failures would change the estimand. The report states 8 against the requested 50. The 2026-08-20 literature pass adds only explicitly post-hoc diagnostics (A/B direction agreement, B-oracle headroom, edit magnitude); none changes tau, max-retry, or the loop. |
| 2026-08-20 | 🔴 **S4.6 independent double-coding WAIVED by Sabbir after he reviewed Coder-A; taxonomy finalized as single-coder, user-endorsed, with no agreement statistic** | Protocol decision 6 required two independent coders and pre-reconciliation agreement. Codex completed Coder-A under Sabbir's explicit authorization. Sabbir then stated that he had read the filled sheet, found it correct, and did not want another annotator. Because endorsement occurred **after seeing Coder-A**, it is not an independent second coding. The final artifact therefore records `independent_coder_b=false`, `agreement=null`, and status `single_coder_user_endorsed_protocol_deviation`; it never reports Sabbir as a coder or invents agreement. | This is Sabbir's explicit resource/method decision, not a literature-backed improvement. It weakens the qualitative failure taxonomy and must be stated in Limitations. A second annotation was not simulated: one model role-playing two coders would preserve neither independence nor the registered rationale for agreement. Before finalization Codex re-read all eight emitted drafts against the axis swap test and corrected BN043 from a post-hoc specificity mismatch to **no observable emitted-text failure**. |
| 2026-08-21 | ⚠️ **Hosted Gemma-4 emitted non-JSON prose after one otherwise valid structured verdict; parser repair and explicit checkpoint migration** | The first 20-base-case S5 chunk stopped during the Gemma-4 judge call after 50 completed condition-case log lines because `json.loads()` rejected extra text following a valid leading JSON object. The repair decodes exactly one leading JSON object, applies the unchanged three-field schema and PASS/FAIL rules, archives any non-JSON suffix verbatim as `trailing_text`, and still rejects a second JSON object or any missing/extra schema field. The clean `9e00d2e` checkpoint is migrated only by a one-source, provenance-preserving script: all existing calls/cases retain their source provenance and are marked `scientific_generation_unchanged`; no Writer generation, prompt, score, or outcome row is altered. | This is a provider-format/runtime repair, not an alteration of the Gemma-4 treatment or its decision rule. Keeping the prefix only is acceptable because it is the sole schema-valid object and the suffix cannot affect it; retaining it makes the exceptional provider behavior auditable rather than silently discarded. Reusing the already written local calls prevents regeneration, while the failed judge request has no accepted verdict row and is retried under the same frozen prompt/model/seed. An implicit cross-commit resume is refused; the named migration is the evidence trail for this narrowly scoped repair. |
| 2026-08-21 | ⚠️ **Gemma-4 returned `status: incomplete` during the next S5 batch; incomplete interactions now archive and retry** | The seed-42 base-case 60–99 batch stopped after 40 completed condition-case log lines when an Interactions response had `status: incomplete`; no verdict, partial text, or score from that response was used. The runner now archives each incomplete raw interaction in a separate append-only transport-failure JSONL, counts it against the local request/token/day limiter, and retries the **same** request at most three times under the unchanged model, prompt, seed, high-thinking setting and 512-token cap. Only a later `status: completed` response that passes the existing exact schema can enter the judge archive. The explicit checkpoint migration accepts only the two named clean commits from the JSON-prefix and transport-retry repairs, preserves source provenance, and never changes generated text or accepted judgement rows. | This is a transport resilience repair, not additional logical judge feedback or a changed row-8 budget: incomplete responses have no usable verdict and transport retries are separately archived rather than counted as `logical_judge_calls`. Retrying without recording would hide a provider failure and undercount quota usage; accepting partial output would be methodologically invalid. Three consecutive incomplete responses stop the run loudly, leaving all completed Writer calls resumable. |
| 2026-08-21 | ⚠️ **S5 row 8 — observed unbounded-feedback cap failure; bounded-enum feedback restart** | Inspection of `s5_checkpoint (2).zip` found all three archived BN047/L0 incomplete responses ended at exactly **512 output tokens**. Each began a JSON FAIL object then repeated Bangla text inside the open `feedback` string; none supplied a usable verdict. The row-8 response schema now constrains `feedback` to an API-supported enum: empty for PASS, or one exact short Bangla correction selected by requested target level for FAIL. The prompt and client validation require the matching template. Kaggle preflight now probes both PASS and FAIL forms before the 12B Writer loads. The exact 512 cap, model, seed, high-thinking setting, rubric, score range and three logical judge attempts remain unchanged. A named migration removes **only** old row-8 cases, judge calls, row-8 Writer retries, and incomplete raw responses from the active archive; it preserves the other nine conditions and shared initials with source provenance, and writes the removed rows to explicit superseded archives. | The raw evidence identifies max-output completion, not a transient provider outage: Google documents `status=incomplete` as including `max_tokens`, and documents `enum` (but not `maxLength`) in its supported structured-output subset. Raising the cap would not constrain the observed repetition and would alter the registered compute surface. Reusing old row-8 outcomes would mix feedback instruments, so row 8 restarts while the unaffected nine conditions do not. This is a documented treatment-interface repair, not a performance tuning action; sealed Verifier-B remains absent from generation and the sole final scorer. |
| 2026-08-21 | 🔧 **S5 feedback-enum migrator accepts both clean pre-repair checkpoint commits** | The first repair notebook encountered a valid `2e919895` checkpoint rather than the later `19fbee01` checkpoint. Both are now explicit source commits: 2e carries the JSON-prefix repair; 19f adds the incomplete-response archive. In both, row 8 still uses the same unbounded feedback contract, so the migrator partitions exactly the same treatment-specific rows and retains the other nine conditions with provenance. | This is resume compatibility, not a new scientific variation. Refusing a known clean checkpoint would force unnecessary regeneration; accepting an unknown commit could silently mix a changed instrument. The allowed source list is deliberately finite and tested. |
| 2026-08-22 | ⚠️ **S5 post-run — Bangla LaBSE-feature MAUVE recorded as a sensitivity analysis, not headline MAUVE** | The raw-text `mauve-text` default uses GPT-2-large features. It is not used as a Bangla semantic result. Instead, MAUVE is computed from precomputed `sentence-transformers/LaBSE` embeddings with all package settings fixed across the 20 condition × level cells. Each comparison has 270 generated and 270 region-A real texts. The output is named **LaBSE-feature MAUVE**, never simply “MAUVE”; the report explicitly retains `generated_sentiment_js=PENDING`. | The MAUVE package supports precomputed features, while its own guidance recommends a few thousand texts per distribution and warns small samples are optimistic/variable. Using an English GPT-2 representation as a Bangla headline would be less defensible than a language-capable embedding sensitivity check, but 270 samples cannot support a definitive distributional claim. Recent diversity work also recommends combining non-redundant measures rather than treating one diversity number as decisive. This changes interpretation, not any generator, prompt, seed, selection, or score. |
| 2026-08-23 | ⚠️ **Phase-8 live demo wrapper built before S6, at Sabbir's explicit request** | The normative pipeline says wrappers are built only after S6. A local interactive wrapper now accepts a Bangla plot and requested axis level, retrieves top-10 same-level exemplars from the frozen R1-only index, generates through hosted `gemma-4-26b-a4b-it`, applies Verifier-A at tau*=0.4384071 plus symbolic diagnostics/feedback for at most three attempts, and displays the full trace. Verifier-B is never loaded. The frozen 5,400 generations and ten-condition results are read-only and were not rerun. | This is an engineering-order deviation, not a change to an estimator or a new thesis result. The UI is labelled diagnostic: its live hosted Gemma-4 Writer differs from the frozen S5 Writer (`google/gemma-3-12b-it`), and plot-faithfulness/hallucination scoring remains an unvalidated draft audit. The wrapper cannot be cited as S5 evidence and does not complete S6. |
| 2026-08-23 | ⚠️ **Live demo adds a source-grounded plot check; it remains outside the scientific loop and frozen results** | A separate hosted `gemma-4-31b-it` call checks the emitted comment against only the user-supplied plot and returns `SUPPORTED`, `REVIEW`, or `UNSUPPORTED`, a short Bangla explanation and explicit unsupported claims. It never changes, rejects or regenerates an output, never sees Gold-300/R2/Verifier-B, and is not persisted. | This answers a product-level need without retrofitting an unregistered metric into S5. Consensus review changed the design from a bare numerical “faithfulness score” to inspectable statement-level triage because automatic factuality metrics remain fragile and require human validation. The interface labels the check operational and not human-validated; the registered post-run human audit remains separate. |
| 2026-08-23 | **Final thesis scope — cross-language RQ removed; verifier divergence renumbered RQ4** | The active thesis has four questions: construct recovery, controllability, neural/symbolic validation and held-out-verifier divergence. The former cross-language RQ is not shown as unanswered or future work, and the old RQ5 becomes thesis RQ4. The completed Bangla experiment, frozen analyses and all numeric results are unchanged. | Sabbir explicitly chose to omit the English work from the current thesis rather than defend an unanswered question. Historical protocol and pipeline text is retained for auditability, but a dated pipeline amendment makes it non-active. This is a reporting/scope deviation, not selective removal of an observed result: no English experimental surface exists and no Bangla result is dropped. |
| 2026-08-25 | ⚠️ **POST-HOC RQ3 contrast added: neural + symbolic feedback versus neural-only; exploratory, never promoted into the nine-comparison family** | The frozen 5,400 Verifier-B scores permit 540 exact pairs on `plot_id × target_level × replicate_seed`. The contrast compares `rag_neural_symbolic_feedback` with `rag_neural_loop`, reports paired target-probability, binary-accuracy, call and token deltas overall and per level, and performs no generation rerun or B rescoring. | The contrast directly addresses RQ3 but was omitted from the registered inferential family. It is being added after the nine zero-shot comparisons are known and after the hybrid condition was observed to have the largest zero-shot effect, so selection may inflate the apparent advantage. The result is therefore labelled post-hoc/exploratory; its interval is explicitly naive after selection, p-values are descriptive and unadjusted, and no hybrid-superiority claim is licensed. The original nine planned comparisons remain unchanged. |
| 2026-08-25 | 📌 **POST-HOC RQ3 outcome: a small scorer-probability difference is Level-0-specific; binary evidence is inconclusive** | Across 540 pairs, hybrid minus neural-only is +0.02159 in Verifier-B target probability (naive post-selection 95% CI [0.00082, 0.04310]) and +0.02037 in binary accuracy; discordant successes are 26 versus 15 (exact McNemar p=0.11728). By level, probability deltas are +0.04328 for Level 0 and -0.00009 for Level 1. | The selected contrast cannot establish general hybrid superiority. It is reported with effect, compute, heterogeneity, and selection caveat together; RQ3 remains mixed and requires a future preregistered direct comparison. No registered result, generation, score, threshold, or confirmatory family was changed. |
