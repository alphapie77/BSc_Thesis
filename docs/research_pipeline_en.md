...
- **Row 2 static-example rule (frozen before generation):** one seed-42 stratified draw of ten R1 region-A examples per target level, after sorting review IDs; the same ten are reused for every plot, language arm, and replicate. There is no query-dependent or learned selection. This preserves row 2 as static few-shot rather than a second retrieval method.
