...
**Weeks 7–9 — Loop (S5)** — ✅ **COMPLETE 2026-08-20.** Phase 4 is where the title's *"Multi-Agent"* and *"Verifier-in-the-Loop"* live. S4.6 dynamics and the complete 8/8 failure census are reported; the 50-case and double-coding shortfalls are explicit deviations, not silently repaired.
☑ 16. Build LangGraph (§4.1–4.2) + generator pilot — **DONE, with a void model comparison recorded rather than repaired post hoc.** The four role-separated components, R1-only index, trace schema, local nf4 Writer, Reflector retries, and length-controlled dev-plot substrate are implemented and tested. The TigerLLM upload proved byte-identical to Gemma-3, so the comparison is void; Gemma-3-12b-it remains the bounded single generator. S4.5a then established no held-out predictive value for the symbolic mixture, so Verifier-A alone gates while symbolic rules remain diagnostic.
☑ 17. τ sweep → operating point [§4.5] → Gate G4 — **RUN 2026-08-20.** ~~first-pass 60–70%~~ remains struck: the registered gain-per-call argmax selects **τ*=0.4384071**, Verifier-B quality **0.802219**, mean **2.000 calls**, first-pass **0.6500**, final acceptance **0.8667**. alpha_lo **0.640501** at one call; explicit `FORCED_3` alpha_hi **0.866272** at five calls; selected fraction of achievable gain **71.63%**. The full observed-score frontier and mandatory per-level views are retained. **Global headline, not hierarchical pooling:** primary reading found the cited paper did not contain the registered variance-ratio estimator; that pre-run correction is recorded in `protocol.md` and STATUS.
...
>
> ⛔ **PILOT RUN 1 IS VOID AS A COMPARISON (2026-08-12, evening).** `TigerLLM-9B-it`'s uploaded weights are `gemma-3-12b-it` **byte-for-byte** (SHA-256 `4847447e…` on shard 1 of both), so both arms ran the same model and 37 of 40 generation pairs came out identical. **Ruling (Sabbir delegated, reasoning Claude's): the pilot is not re-run; the generator is fixed at `google/gemma-3-12b-it`, single-arm, and the Bangla-adapted comparison becomes a pre-registered robustness check** (open decision 22). The single-backbone bound is stated in Ch.5 Limitations citing `2604.04532`. 🎁 Salvage: the accidental double-run is a **passed determinism check** — identical seed → identical generations across two loads, which the hosted API could not have given. Measured on the 80 valid generations: truncation **2/80**, mean **22 words** (corpus median 8), non-Bangla-script leakage **1/20 under Bangla prompts vs 4/20 under English prompts** — the first data point for the §3e prompt-language factor.
...
- **Scale:** per replicate, **90** eval-plots (never dev-plots) × **2 axis levels** × **10 conditions = 1,800 condition-cases per language**. Three pre-declared sampling replicates (`42, 43, 44`) make **5,400/language and 10,800 across both languages**. These are not generation-call counts: rows 4–9 are multi-call and every realized input/output token count is logged. Replicates are a sensitivity/blocking factor, never a best-seed selection surface. 🔴 **Amended 2026-08-20 before Phase-5 generation:** decisions 9 and 11 added two strong controls after the 2025–26 literature audit.
- **Row 2 non-retrieved few-shot rule (amended before generation):** draw ten R1 region-A examples per target level independently for each plot × level × replicate, using a SHA-256-derived RNG from `(42, replicate_seed, plot_id, target_level)` after sorting review IDs. The draw never uses plot text, similarity, an outcome, R2 or G. Thus it is instance-randomized few-shot, not retrieval; the full schedule digest is frozen at preflight. This supersedes the same-day fixed-set rule after Li et al. (EMNLP 2025) showed fixed random-factor settings can bias comparisons and instance-level randomization reduces variance.
- **Row 5 symbolic gate (frozen before generation):** on the 60 frozen length-controlled Bangla dev attempt-1 cases, choose the symbolic threshold whose acceptance rate is closest to row 6's observed neural first-pass rate (39/60 = 0.65), breaking an exact-distance tie toward the stricter threshold. This gives **tau_symbolic = 0.18166513482099075** and exactly **39/60** passes. Verifier-B is not used.
- **Row 8 judge contract (amended 2026-08-21 before generation):** `gemini-3.6-flash` through the v1beta Interactions API, with `seed=42` and explicit `thinking_level=medium`. Gemini 3.6 no longer accepts a temperature parameter. Structured JSON contains `verdict ∈ {PASS, FAIL}`, integer `target_fit_score ∈ [0,100]`, and Bangla `feedback` of at most two sentences (empty on PASS). The judge sees the exact axis definition, requested level, plot, and draft; it never sees Verifier-A/B. At most three Writer drafts are attempted. If all fail, emit the highest judge score, breaking ties toward the earliest draft. Raw responses, exact model ID, seed, thinking level, token usage, and latency are retained; Verifier-B scores final text only. This supersedes the inaccessible `gemini-2.5-flash` contract after Google's live endpoint told this new-user account to migrate; no eval generation existed when amended.
...
- **Paired call schedule:** rows 3–9 share one physical, byte-identical initial RAG draft per plot/level/replicate, while each condition retains that call in its logical cost. Rows 4 and 6 use the same number of Reflector calls and the same feedback renderer; row 4 supplies an empty diagnostic list and row 6 supplies symbolic rule names. Physical reuse, logical cost, per-call RNG seed, and token totals are all archived separately.
- **Checkpoint compatibility:** before any model load, every restored S5 JSONL must parse completely, have unique keys, and identify the exact clean runner commit. A runtime RAG rebuild must match the committed R1-only manifest's nine scientific invariants without rewriting that tracked artifact; the config-relative persistence path is host-local and not an identity field. A dirty or cross-commit checkpoint is refused rather than mixed into one result table.
...
- **Exactly 3 pre-declared sampling replicates (`42,43,44`) per arm.** All comparisons are paired within plot/level/replicate. Replicate mean±SD is descriptive sensitivity only, never the decision rule or a best-seed selection surface; primary inference is paired at case level with replicate retained as a blocking factor.
...
**Deliverables:** master results table (10 × 2 languages), Goodhart figures (incl. A−B vs attempt), realism figure, cross-lingual Δ table, mini-ablation table, stats appendix.
