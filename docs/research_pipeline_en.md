...
- **Row 8 judge contract (frozen before generation):** `gemini-2.5-flash`, temperature 0, structured JSON with `verdict ∈ {PASS, FAIL}`, integer `target_fit_score ∈ [0,100]`, and Bangla `feedback` of at most two sentences (empty on PASS). The judge sees the exact axis definition, requested level, plot, and draft; it never sees Verifier-A/B. At most three Writer drafts are attempted. If all fail, emit the highest judge score, breaking ties toward the earliest draft. Raw responses, exact model ID, token usage, and latency are retained; Verifier-B scores final text only.
