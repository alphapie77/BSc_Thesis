...
- **Checkpoint compatibility:** before any model load, every restored S5 JSONL must parse completely, have unique keys, and identify the exact clean runner commit. A runtime RAG rebuild must equal the committed R1-only manifest without rewriting that tracked artifact. A dirty or cross-commit checkpoint is refused rather than mixed into one result table.
