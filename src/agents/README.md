...
| 3 | `critic.py` | **no, deliberately** | Verifier-A vs τ decides PASS/FAIL; symbolic scoring remains active for Reflector diagnostics |
...
3. **τ has no default.** It is a required argument selected by the registered
   quality–cost frontier. S4.5a retired `w`; symbolic evidence cannot move the
   verdict after adding zero held-out predictive value in every fold.
...
**Both Critic scorers were fitted on human reviews and are applied to generated
text.** `kapur2026length` shows the length/specificity relation is flat or
reversed in machine text. S4.5a measured that shift and found no held-out value
from the symbolic scorer; it remains a diagnostic channel, not a gate.
